...
class Blockchain {
  ...
  _addBlock(block) {
    ...
    return new Promise(async (resolve, reject) => {
      ...
      try {
        ...
        reject("Adding a new block failed!");
      }
    });
  }
  ...
  getBlockByHash(hash) {
    let self = this;
    return new Promise((resolve, reject) => {
      let blockFilterArray = self.chain.filter((item) => {
        return item.hash === hash;
      });
      let block = blockFilterArray[0];
  ...
  validateChain() {
    ...
    return new Promise((resolve, reject) => {
      self.chain.map(async (block, index) => {
        ...
        if (index + 1 < self.chain.length) {
          if (self.chain[index + 1].previousBlockHash != block.hash) {
            errorLog.push(
              "Issue with block " +
                (index + 1) +
                ". Previous hash does not match with the previous block's hash"
            );
          }
        }
      });
      if (errorLog.length != 0) resolve(errorLog);
      else resolve("Your blockchain is absolutely healthy");
    });
  }
}
...
/*
{
	"address":"mt6Kmudc2x5W1fpkXqdqWMCvW18en33tav",
	"signature":"HyJBAWYf8z29BUzyDCxG/YSlatCEddVGjGjDbgrkX+EYOz+2xLfVbDHRotgshxsnbRkIQK5vIULufo6bL3BIRHI=",
	"message":"mt6Kmudc2x5W1fpkXqdqWMCvW18en33tav:1640555063:starRegistry",
	"star":{
		"dec":"68 52 56.9",
		"ra":"16h 29m 1.0s",
		"story": "Lohit is here!"
